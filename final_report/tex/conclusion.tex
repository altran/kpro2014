...
\subsection{Hardware-based prototype (with no mock up data)}
...
Another improvement that can be made is improving the look of the visualiser itself. None of us are graphics artists, and the visualisation is quite simple in it’s looks. It could be better looking, by adding effects such as shadows and drawing our sensors in a more aesthetically pleasing fashion. It could also be visualised in three dimensions, where we would be able to see the depth of the system as well. While this would definitely add complexity to the system, it would also mean that we could completely map an entire area and correctly show it as is.
