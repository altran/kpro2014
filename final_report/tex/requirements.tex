...
\\ \hline 2.1
&Request data from sensor
&The sensor data will be collected by the server, this is done by central hub requesting data from the sensors every few seconds (the time is set from the sensor side). The data format is in raw data format.
\\ \hline 2.2
&Store data
...
The sensor is provided by the customer. We do not know how the sensor interact with environment to get the data, but it does not concern us. We will focus on how the sensors interact with central hub. Still the sensor is the basic of the system and gathering those data is a must have requirement in order to make the whole system work.
...
\\ \hline 3.1
&Gathering data
&The sensor will record 4 types environmental data, temperature ,pressure , humidity and lighting. Those 4 will then be saved until next recording.
\\ \hline 3.2
&Send data
...
\\ \hline 4.2
&Show the data on the GUI
&The data will be displayed inside a window by different elements that would reflect data status. For example there will be a colour showcasing temperature and that colour should turn more red if the temperature increases, to blue if the temperature decreases.
\\ \hline 4.2.1
...
This is the view where all sensors and the data is showcased in a table. The sensor number can be seen on the left and the data from the sensors can be seen in the appropriate row. If the user prefers less information he can click on the check box on the right to hide information that is unwanted.
...
The map view will make an illustration of the room. In the real model the data will be shown in the form of image manipulation, not a function in the mock-up program. If position can be tracked we will do so, otherwise we must find another way to show the data in a 2D map.
...
&The administrator can configure a text file containing scaling for link budget, meaning how large the link budget needs to be before we consider the link budget as out of range.
\\ \hline 5.3
