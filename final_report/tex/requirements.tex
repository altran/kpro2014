...
\\ \hline 2.1
&Request data from sensor
&The sensor data will be collected by the server, this is done by central hub reuqesting data from the sensors every few secounds (the time is set from the sensor side). The data format is dash7.
\\ \hline 2.2
&Store data
...
\\ \hline 3.2
&Send data
&The sensor will send data to the central hub if requested, the format will be in dash7.  
\\ \hline 
\end{tabularx}
\end{table}
