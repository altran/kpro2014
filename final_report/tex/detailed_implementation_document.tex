...
\begin{tabularx}{\textwidth}{|L{1.2cm}|L{3.5cm}|X|}
	...
	\\ \hline RMF1
	&sensorModel List
	&Room model will keep an ArrayList of SensorModel objects in order to update each sensor
	\\ \hline RMF2
	&getSensorModel(int ID)
	&A method to get a specific SensorModel using the sensor id
	\\ \hline RMF3
	&getSensorNumber()
...
\begin{tabularx}{\textwidth}{|L{1.2cm}|L{3.5cm}|X|}
	...
	&Creates a dataTable which is a table view that is filled with data from the data list. Also adds check boxes for the columns and listeners to make it possible to choose which columns should be visible in the dataTable.
	\\ \hline BVF3
	&main(String[] args)
...
\begin{tabularx}{\textwidth}{|L{1.2cm}|L{3.5cm}|X|}
	...
	\\ \hline IF5
	&Check
	&Nearly all (except the centralHub) instruction comes with a boolean check, which is for the check boxes. If the check box is unchecked the boolean will be false and the animation will be hidden. If the check box is checked the animation will continue.
	\\ \hline IF6
	&Specific function
...
\begin{tabularx}{\textwidth}{|L{1.2cm}|L{3.2cm}|X|}
	...
	&Add “radiogateways” to the queryResource path, and split the returned \gls{DASH7} String so that only a list of radiosensor ids remain, and return it.
	\\ \hline DBM6
	...
	&Query the database for latest data for a sensor id using a POST request with query parameter (“query”, “radiosensor:”) . Then return the observation with most recent Date value.
	\\ \hline DBM9
...
\caption{Class Diagram for Mock-Up Data Based Prototype}
...
There are two classes appended to the visualiser module: Constants and Calculation. The constants class serves as a loading point to make the code more modifiable. While it is not used in our prototype to its full extent, it can hold fields for any configurable visualiser property. It allows reading from a user-generated visualiser.properties file, and provides sensible defaults if no such file is provided. In our prototype, we use it to set the database IP address, and to calibrate values of the link budget.
...
In addition to the two classes in the visualiser, four classes were added to the database module: ManualAddDash7Data, Dash7Generator, WalledRoomGenerator and DefaultGenerator. The class ManualAddDash7Data is used to add mock-up data to a running database. It is executed as a stand-alone Java program, and contains only static fields.

The class ManualAddDash7Data relies on the three generator classes. The relationship between the generator classes is that Dash7Generator is an interface realised by WalledRoomGenerator and DefaultGenerator. The Dash7Generator interface defines the type of an object that generates strings of sensor data ready to be posted to the database. The class WalledRoomGenerator defines a generator object for generating sensor data simulating an environment with several isolated rooms, while the class DefaultGenerator defines a similar generator object for one room only.
...
The visualiser now uses the separate class Calculation to compute the screen position of gateways and sensors based on the link budget. In order to make the link budget calculation configurable, the calculation involves fields from the class Constants, which again initialises the fields from properties in the file “visualiser.properties”. The animation works the same way as before, using the class AnimationTimer and the interfaces Instruction and Renderer.
...
CC1
&Constants
&This method will read from a text file that contains maxLinkBudget and minLinkBudget and set those two fields.
\\ \hline 
\end{tabularx}
\end{table}
...
\\ \hline DBM11
&getAllGatewaysIDs()
&Query the IoT Service database using radiogateways as path, and sort out the gateway ids from the returned result.
\\ \hline DBM12
...
\\ \hline MADD4
&manualDataPost()
&The user writes observation data manually in the console. This mode is meant for copypasting observations into the console.
\\ \hline MADD5
&readFromFile()
