...
As with some of the other requirements, maintainability is not a high priority. Our customer is to decide what they will be using these sensors for in the future, and will provide maintainability for the system when it is necessary. Regarding our project, we do ot require any maintainability.

\paragraph{Portability} \ \\
The system needs to be portable. In other words, it should work on any platform. Using Java we will contribute to the overall portability of the system, however, we may also limit the system to certain hardware, as some hardware may not have all of the capabilities, or libraries, of the Java version that we are using.
...
\paragraph{Reliability} \ \\
Our prototype needs to work for a total of one day, or a total of twenty-four hours. It is desirable, but not necessary, for the prototype to run for more than one working day.
...
\paragraph{Portability} \ \\
If our system is able to work in an environment other than the development environment, then we have satisfied the portability metric.
